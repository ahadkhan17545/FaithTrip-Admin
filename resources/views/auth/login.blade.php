...
@section('content')
    <div class="left_section w-100">
        ...
        <form action="{{url('login')}}" class="register-form" method="POST">
            @csrf
            ...
            {{-- <a href="/admin/search.html" class="btn btn_warning text-white w-100">Sign in</a> --}}
            <button type="submit" class="btn btn_warning text-white w-100">{{ __('Login') }}</button>

        </form>
    </div>
    <div class="right_section">
        <img src="{{ url('assets') }}/img/biman.png" alt="Login image" />
    </div>
@endsection
