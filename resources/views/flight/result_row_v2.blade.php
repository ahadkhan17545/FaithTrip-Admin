...
<div class="row flight_card">
    <div class="col-lg-2 flight_airlines">
        <img class="img-fluid" src="{{ url('airlines_logo') }}/{{ strtolower($data['pricingInformation'][0]['fare']['validatingCarrierCode']) }}.png" alt="{{ strtolower($data['pricingInformation'][0]['fare']['validatingCarrierCode']) }}" loading="lazy">
        @if($airlineInfo)<h5 class="text-center">{{$airlineInfo->name}}</h5>@endif
    ...
    <div class="col-lg-4 flight_duration">
        ...
        <button>@if(count($segmentArray) > 1) {{count($segmentArray)-1}} @else Non @endif Stop</button>
    </div>
    <div class="col-lg-2 flight_timing">
